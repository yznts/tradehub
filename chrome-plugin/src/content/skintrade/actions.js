...
function ExtractNames(callback) {
    var needed_items = [];
    $(".offer_container_invertory").each(function (_, element) {
        needed_items.push($(element).attr("market_hash_name"));
    });
...
function UpdateView(callback) {
    $(".offer_container_invertory").each(function (index, item) {
        ...
        // Extract info
        var info = cache[$(item).attr("market_hash_name")];
        if (!info) {
            return;
        }
...
function SortS1S2(callback) {
    $("#inventory_bots").find(".offer_container_invertory").sort(function (a, b) {
