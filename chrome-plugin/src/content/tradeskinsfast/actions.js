...
function ExtractNames(callback) {
    ...
    $(".item").each(function (_, element) {
        var name = $(element).find(".pic").find("img").first().attr("alt");
        if (name === undefined)
            return;
        needed_items.push(name);
    });
...
function UpdateView(callback) {
    $(".item").each(function (index, item) {
        ...
        // Extract name
        var name = $(item).find(".pic").find("img").first().attr("alt");
        if (name === undefined)
            return;
        ...
        rates_node.find(".s1-price").html(info["s1-price"].toFixed(2)+"$");
