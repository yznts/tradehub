...
function ExtractNames(callback) {
    ...
    $(".item").each(function (_, element) {
        needed_items.push($(element).attr("data-original-title"));
    });
...
function UpdateView(callback) {
    $(".item").each(function (index, item) {
        ...
        // Extract info
        var info = cache[$(item).attr("data-original-title")];
        if (!info) {
            return;
        }
...
function SortS1S2(callback) {
    $("#sinv-loader > ul").find(".item").sort(function (a, b) {
