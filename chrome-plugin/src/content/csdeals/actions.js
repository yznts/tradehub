...
function ExtractNames(callback) {
    ...
    $(".item").each(function (_, element) {
        var i_name = $(element).find(".pic").find("img").attr("alt");
        needed_items.push(i_name);
    });
...
function UpdateView(callback) {
    $(".item").each(function (index, item) {
        ...
        // Extract name
        var i_name = $(item).find(".pic").find("img").attr("alt");
...
function SortS1S2(callback) {
    $("#botinventory").find(".item").sort(function (a, b) {
