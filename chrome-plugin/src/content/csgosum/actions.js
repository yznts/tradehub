...
function ExtractNames(callback) {
    var needed_items = [];
    $(".inventory-item-hold").each(function (_, element) {
        needed_items.push($(element).attr("data-item-name"));
    });
...
function UpdateView(callback) {
    $(".inventory-item-hold").each(function (index, item) {
        ...
        // Extract info
        var info = cache[$(item).attr("data-item-name")];
        if (!info) {
            return;
        }
...
function SortS1S2(callback) {
    $(".bot-results").find(".inventory-item-hold").sort(function (a, b) {
