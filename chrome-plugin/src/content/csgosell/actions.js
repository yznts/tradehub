...
function ExtractNames(callback) {
    var needed_items = [];
    $(".itemImgDiv").each(function (_, element) {
        needed_items.push($(element).attr("original-name"));
    });
...
function UpdateView(callback) {
    $(".itemImgDiv").each(function (index, item) {
        ...
        // Extract info
        var info = cache[$(item).attr("original-name")];
        if (!info) {
            return;
        }
...
function SortS1S2(callback) {
    $("#botsIv").find(".itemImgDiv").sort(function (a, b) {
