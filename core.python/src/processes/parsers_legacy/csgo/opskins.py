...
class CsgoOpskinsParserProcess(multiprocessing.Process):
    ...
    def run(self):
        while True:
            try:
                ...
                # Request new data
                resp = self.http.get('https://api.opskins.com/IPricing/GetAllLowestListPrices/v1?appid=730')
                ...
                self.r_meta.hset("last-updates", "{0}-{1}".format('csgo', 'opskins'), int(time.time()))

                # Log
                logging.info('({0:.2f}) CS:GO opskins updated'.format(time.time() - start_time))
